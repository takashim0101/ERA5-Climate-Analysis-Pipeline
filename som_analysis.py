...
# # # # 
# # # # try:
# # # #     ds_temp_wind = xr.open_mfdataset(file_pattern_temp_wind, combine='by_coords', chunks={'time': 24})
# # # #     ds_precip = xr.open_mfdataset(file_pattern_precip, combine='by_coords', chunks={'time': 24})
# # # #     
# # # #     # Merge and rename variables for clarity
...
# # # # 
# # # # # 3. Total Precipitation (CRITICAL FIX: Cumulative to Incremental)
# # # # precip_incremental = ds['tp'].diff(dim='time', label='lower') * 1000
...
# # # # monthly_temp = ds['temperature_c'].resample(time='1M').mean()
# # # # monthly_precip = ds['total_precipitation_mm_inc'].resample(time='1M').sum() 
# # # # monthly_wind = ds['wind_speed'].resample(time='1M').mean()
# # # # 
# # # # 
...
# # # #     # Compute mean and std across all time and spatial points
...
# # # #             # Select and flatten monthly features for this grid cell
# # # #             # method='nearest' is used here, ensuring a value is found for the grid cell
# # # #             features = scaled_var.sel({lat_dim: lat_val, lon_dim: lon_val}, method='nearest').values
# # # #             combined_features.extend(features)
# # # #         
# # # #         combined_features = np.array(combined_features)
# # # #         
# # # #         # Ensure no NaN values (critical for SOM)
...
# # # # 
# # # # som = SOM(m=som_m, n=som_n, dim=som_dim, num_iteration=som_iterations, random_state=42)
# # # # som.fit(som_input_data)
# # # # 
# # # # # Get the cluster assignment for each sample (grid cell)
...
# # # # # Use a normalization based on the cluster range (0 to m*n-1)
...
# # # # plt.show()
# # # # 
# # # # print(f"SOM clustering map saved to '{plot_filename}'.")
...
# # #     print("Variables in the merged dataset before renaming:")
# # #     # Print keys to easily see the variable names available in the file
# # #     print(list(ds.variables.keys())) 
# # #     # ----------------------------------
# # # 
# # #     # Merge and rename variables for clarity (THIS SECTION CONTAINS THE ERROR)
# # #     # The keys '2m_temperature', 'total_precipitation', etc., are the source of the error.
# # #     ds = ds.rename({'2m_temperature': 't2m', 
# # #                     'total_precipitation': 'tp', 
# # #                     '10m_u_component_of_wind': 'u10', 
# # #                     '10m_v_component_of_wind': 'v10'})
...
# # #     
# # # except Exception as e:
# # #     # Exit gracefully if loading fails before renaming
# # #     print(f"Error occurred while loading ERA5 data: {e}")
# # #     print("Please ensure files exist in 'downloaded_data' and are not corrupted.")
# # #     exit()
# # # 
...
# # # 
# # # # 3. Total Precipitation (CRITICAL FIX: Cumulative to Incremental)
# # # # This uses the 'tp' name defined in ds.rename, which will fail if renaming fails above.
# # # # We proceed for completeness, but this section won't be reached if the rename error occurs.
# # # precip_incremental = ds['tp'].diff(dim='time', label='lower') * 1000
# # # precip_incremental = precip_incremental.fillna(0)
# # # ds['total_precipitation_mm_inc'] = precip_incremental
# # # 
...
# # # for var in feature_vars:
# # #     # Compute mean and std across all time and spatial points
# # #     # .compute() is used because Dask arrays are likely in the dataset
# # #     var_mean = var.mean().compute()
# # #     var_std = var.std().compute()
# # #     # Apply standard scaling
# # #     scaled = (var - var_mean) / var_std
...
# #     ds = xr.merge([ds_temp_wind, ds_precip])
# #     
# #     # The variable renaming block is REMOVED as the short names ('t2m', 'tp', 'u10', 'v10')
# #     # are already present in the dataset.
# #     
# #     print("Overview of the ERA5 dataset:")
# #     # The output of ds.variables.keys() would be ['t2m', 'u10', 'v10', 'number', 'valid_time', 'latitude', 'longitude', 'expver', 'tp']
# #     print(ds)
# #     
# # except Exception as e:
# #     print(f"Error occurred while loading ERA5 data: {e}")
...
# # # --- Unit Conversions and Variable Creations ---
# # 
# # # 1. Temperature (K -> C) - Now uses 't2m' directly
# # ds['temperature_c'] = ds['t2m'] - 273.15
# # 
# # # 2. Wind Speed (m/s) - Now uses 'u10' and 'v10' directly
# # ds['wind_speed'] = np.sqrt(ds['u10']**2 + ds['v10']**2)
# # 
# # # 3. Total Precipitation (CRITICAL FIX: Cumulative to Incremental)
# # # Uses 'tp' directly.
...
#     ds_temp_wind = xr.open_mfdataset(file_pattern_temp_wind, combine='by_coords', chunks={'valid_time': 24})
#     ds_precip = xr.open_mfdataset(file_pattern_precip, combine='by_coords', chunks={'valid_time': 24})
#     
#     # Merge datasets
#     ds = xr.merge([ds_temp_wind, ds_precip])
#     
#     # NOTE: Variable renaming is REMOVED because xarray/cfgrib already assigned short names:
#     # '2m_temperature' -> 't2m', 'total_precipitation' -> 'tp', etc.
#     
#     print("Overview of the ERA5 dataset:")
#     print(ds)
#     
...
# 
# # 3. Total Precipitation (CRITICAL FIX: Cumulative to Incremental)
# # Use 'tp' directly. Multiply by 1000 for meters to millimeters.
...
# 
# # --- Calculate Monthly Means/Sums ---
# # FIX: Use 'valid_time' dimension for resampling
# monthly_temp = ds['temperature_c'].resample(valid_time='1M').mean() 
# monthly_precip = ds['total_precipitation_mm_inc'].resample(valid_time='1M').sum() 
# monthly_wind = ds['wind_speed'].resample(valid_time='1M').mean()
# 
# 
# # --- Prepare SOM Input Array and Standardization ---
# som_input_data_list = []
...
# 
# try:
#     # 1. Load data without specifying time-based chunks initially to avoid warnings
...
#     
#     # 2. Merge datasets (using compat='no_conflicts' to suppress FutureWarning)
#     ds = xr.merge([ds_temp_wind, ds_precip], compat='no_conflicts')
# 
#     # 3. CRITICAL DASK OPTIMIZATION: Rechunk the data by month 
#     # (or a small time-chunk size) to optimize the subsequent resample operation.
#     # 744 hours is roughly a month (31 days * 24h). 
#     # This aligns chunks with the resampling period.
...
# ds['wind_speed'] = np.sqrt(ds['u10']**2 + ds['v10']**2)
# 
# # 3. Total Precipitation (Cumulative to Incremental)
# # Use 'tp' directly. Multiply by 1000 for meters to millimeters.
# precip_incremental = ds['tp'].diff(dim='valid_time', label='lower') * 1000
# precip_incremental = precip_incremental.fillna(0)
# ds['total_precipitation_mm_inc'] = precip_incremental
...
# 
# # --- Calculate Monthly Means/Sums ---
# # FIX: Use 'valid_time' dimension and 'ME' alias (Month End) for resampling
...
# scaled_features = []
# for var in feature_vars:
#     # This .compute() is necessary and is the heavy part. 
#     # The rechunking above should make this run faster and use less memory.
#     var_mean = var.mean().compute()
#     var_std = var.std().compute()
#     # Apply standard scaling
#     scaled = (var - var_mean) / var_std
